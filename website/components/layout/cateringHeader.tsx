	...
	useEffect(() => {
		if (data) {
			if (data.siteControls.showVenues && data.siteControls.showPartners) {
				...
				tempNav.push({ name: "Venues", href: "/catering/venue" });
				tempNav.push({ name: "Partners", href: "/catering/partner" });
				setNavigation([...tempNav]);
			} else if (data.siteControls.showVenues) {
				const tempNav = [...navigation];
				tempNav.push({ name: "Venues", href: "/catering/venue" });
				setNavigation([...tempNav]);
			} else if (data.siteControls.showPartners) {
				const tempNav = [...navigation];
				tempNav.push({ name: "Partners", href: "/catering/partner" });
				setNavigation([...tempNav]);
			}
