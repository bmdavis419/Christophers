...
export default function MenuFeatures(props: PropsInterface) {
   ...
   for(let i:number = 0; i < numFeatures; i++) {
    circles.push(<div onClick={(e) => {setActiveFeature(e,i)}} className={`rounded-full mt-auto m-2 md:m-3 p-3 md:p-5 bg-${activeFeature == i ? "primary": "gray-400"}`}></div>)
    }
    ...
    return (
        <div className="overflow-hidden relative md:rounded-50px w-full  md:mt-16 md:h-.4vw align-middle grid md:grid-cols-2 justify-evenly items-center filter drop-shadow-xl bg-gray-50">
            ...
            <div className="space-y-2 lg:space-y-8 my-16 flex flex-col text-center w-2/3 mx-auto">
                ...
                <div className="w-1/2 m-auto lg:text-3xl rounded-full py-2 lg:py-6 bg-primary text-white">Order Online</div>
