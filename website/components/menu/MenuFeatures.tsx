    ...
    return (
        <div className="overflow-hidden relative md:rounded-50px w-full sm:h-1/2vw md:h-1/3vw md:mt-16 grid sm:grid-cols-2 justify-evenly filter drop-shadow-xl bg-gray-50">
            ...
            <div className="text-white sm:text-black sm:pt-12 bg-black bg-opacity-60 sm:bg-transparent sm:bg-opacity-100 p-4 space-y-2 xl:space-y-4 2xl:space-y-8 h-4/5vw flex flex-col text-center lg:w-2/3 mx-auto">
                <Image className="relative visible sm:invisible mix-blend-darken" objectFit="fill" layout="fill" src="/LogoRes.jpg" alt="Menu Header Background Image"/>
                <h1 className="relative mix-blend normal text-3xl xl:text-4xl 2xl:text-5xl font-bold">{Feature.name}</h1>
                <h2 className="relative mix-blend normal text-md xl:text-xl 2xl:text-3xl">{Feature.type}</h2>
                <p className="relative mix-blend normal text-sm xl:text-xl 2xl:text-3xl">{Feature.menuItem.description}</p>
                <div className="relative mix-blend normal w-40 md:w-1/2 m-auto 2xl:text-3xl rounded-full py-2 2xl:py-6 bg-primary text-white">Order Online</div>
