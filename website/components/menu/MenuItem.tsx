...
export default function MenuItem(props: PropsInterface) {
	...
	return (
		<div className="w-36 h-48 md:w-72 md:h-96 filter drop-shadow-2xl rounded-2xl flex flex-col overflow-hidden items-center bg-gray-100">
			<div className="md:hover:bg-black md:group md:hover:bg-opacity-60 relative w-36 h-36 md:w-72 md:h-72"><Image className="mix-blend-darken" objectFit="fill" layout="fill" src="/LogoRes.jpg" alt={name}/>
			<div className="md:mx-8 hidden md:block text-sm md:text-lg mt-6 md:mt-12 text-center absolute opacity-0 md:group-hover:opacity-100 w-36 h-36 md:w-60 md:h-60 top-2 text-white">{description}</div>
			</div>
